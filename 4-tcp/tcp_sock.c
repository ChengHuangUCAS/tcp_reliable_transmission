...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	list_for_each_entry(tmp, list, hash_list) {
		if (tmp->sk_sip == saddr && tmp->sk_dip == daddr && \
				tmp->sk_sport == sport && tmp->sk_dport == dport){
			return tmp;
		}
	}
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	tsk->sk_sip = longest_prefix_match(tsk->sk_dip)->iface->ip;
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
	...
	// log(DEBUG, "tcp_sock_accept: accept_backlog = %d", tsk->accept_backlog);

	if (tsk->accept_backlog == 0) {
		sleep_on(tsk->wait_accept);
	}

	// log(DEBUG, "tcp_sock_accept: waked up. accept_backlog = %d", tsk->accept_backlog);
...
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int size)
{
	...
	while (ring_buffer_empty(tsk->rcv_buf) && tsk->state != TCP_CLOSE_WAIT) {
		pthread_mutex_unlock(&tsk->wait_recv->lock);
		sleep_on(tsk->wait_recv);
		pthread_mutex_lock(&tsk->wait_recv->lock);
	}
	// log(DEBUG, "tcp_sock_read: free buffer: %d", ring_buffer_free(tsk->rcv_buf));
	int read_n = read_ring_buffer(tsk->rcv_buf, buf, size);
	// log(DEBUG, "tcp_sock_read: read size: %d", read_n);
	pthread_mutex_unlock(&tsk->wait_recv->lock);
...
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int size)
{
	...
	int send_size = min(size, ETH_FRAME_LEN - hdr_size);

	while (send_size > 0) {
		int inflight = tsk->snd_nxt - tsk->snd_una;
		tsk->snd_wnd = min(tsk->adv_wnd, tsk->cwnd);
		while ((int)(tsk->snd_wnd - inflight) <= 0) {
			sleep_on(tsk->wait_send);
			inflight = tsk->snd_nxt - tsk->snd_una;
			tsk->snd_wnd = min(tsk->adv_wnd, tsk->cwnd);
		}
		send_size = min(send_size, tsk->snd_wnd - inflight);

		// log(DEBUG, "tcp_sock_write: send window - %d, in-flight - %d, send size - %d", tsk->snd_wnd, inflight, send_size);
		// log(DEBUG, "seq: %d, size: %d", tsk->snd_nxt, send_size);

		char * packet = (char *)malloc(sizeof(char) * (send_size + hdr_size));
		memcpy(packet + hdr_size, buf, send_size);

		tcp_send_packet(tsk, packet, send_size + hdr_size);
		// tsk->inflight += send_size;
		// log(DEBUG, "packet sent");


		buf += send_size;
		size -= send_size;
		send_size = min(size, ETH_FRAME_LEN - hdr_size);
	}
