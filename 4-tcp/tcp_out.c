...
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) 
{
	...
	struct pending_pkt *pkt = (struct pending_pkt *)malloc(sizeof(struct pending_pkt));
	...
	if (list_empty(&tsk->send_buf)) {
		log(DEBUG, "send new packet, reset retrans timer");
		tcp_set_retrans_timer(tsk);
	}
	list_add_tail(&pkt->list, &tsk->send_buf);
	pthread_mutex_unlock(&tsk->wait_send->lock);

	// log(DEBUG, "add seq %d::%d to send buffer", tcp->seq, seq);


	// log(DEBUG, "sent a packet. s %x:%d, d %x:%d, seq %d, len %d, ack %d, rwnd %d", 
	// 	saddr, sport, daddr, dport, seq, tcp_data_len, ack, rwnd);
...
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags)
{
	...
	if (flags & (TCP_SYN|TCP_FIN)) {
		struct pending_pkt *pkt = (struct pending_pkt *)malloc(sizeof(struct pending_pkt));
		pkt->packet = (char *)malloc(pkt_size);
		memcpy(pkt->packet, packet, pkt_size);
		pkt->len = pkt_size;
		pkt->retrans_time = 0;

		pthread_mutex_lock(&tsk->wait_send->lock);
		if (list_empty(&tsk->send_buf)){
			log(DEBUG, "send new control packet, reset retrans timer");
			tcp_set_retrans_timer(tsk);
		}
		list_add_tail(&pkt->list, &tsk->send_buf);
		pthread_mutex_unlock(&tsk->wait_send->lock);
	...
	// log(DEBUG, "sent a control packet. s %x:%d, d %x:%d, snd_nxt %d, rcv_nxt %d, rcv_wnd %d, flags %x", 
	// 	tsk->sk_sip, tsk->sk_sport, tsk->sk_dip, tsk->sk_dport, tsk->snd_nxt, tsk->rcv_nxt, tsk->rcv_wnd, flags);
